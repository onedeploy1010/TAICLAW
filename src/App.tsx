 ...
 *   Path:  /r/{walletAddress}  (generated by profile page)
 *   Query: ?ref={walletAddress}
 */
function getRefCodesFromUrl(): { refCode: string | null; placementCode: string | null } {
  // 1. Try path format: /r/<walletAddress> or /r/<ref>/<placement>
  const pathMatch = window.location.pathname.match(/^\/r\/([^/]+)(?:\/([^/]+))?/);
  if (pathMatch) {
    const urlRef = pathMatch[1];
    const urlPlacement = pathMatch[2] || urlRef;
    localStorage.setItem("taiclaw_ref_code", urlRef);
    localStorage.setItem("taiclaw_placement_code", urlPlacement);
    // Redirect to home so the route doesn't stay as /r/...
    window.history.replaceState({}, "", "/");
    return { refCode: urlRef, placementCode: urlPlacement };
  }

  // 2. Try query format: ?ref=<walletAddress>
  ...
  // 3. Fall back to localStorage (returning user who hasn't registered yet)
  return {
